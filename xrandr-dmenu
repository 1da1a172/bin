...
function main() {
  ...
  iteration=0
  max_iteration=$[ ${(w)#OUTPUT_CONFIGURED} - 1 ]
  xrandr_cmd='xrandr'
...
below ${anchor}
mirror ${anchor}
off" | dmenu -l 10 -p "Select position of $1:" | read selection
...
function select_mode() {
  ...
  if [[ "${OPT_MODE}" == 'auto' ]]; then
    ...
    echo "${menu}" | dmenu -l 10 -p "Select mode for $1:" | read selection
...
function select_rotation() {
  ...
  if [[ "${OPT_ROTATION}" == 'normal' ]]; then
    ...
    echo "normal\ninverted\nleft\nright" \
      | dmenu -l 10 -p "Select rotation of $1:" \
      | read selection
    echo "--rotate ${selection}"
  fi
}
