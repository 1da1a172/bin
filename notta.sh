...
ROM=pa_d2lte-4.3-BETA3-notta-20140508.zip
OUTDIR=~/Dropbox/android/d2vzw/kk-theme
BUILD=pa-4.3b3-notta-20140508
...
#sed -i '/PagerTabStrip/,+1 s/1060012/106000b/' $FILE
...
apktool-trds b Camera2 2>&1 | grep -v 'has no default translation' 1>&2
...
for i in `echo $APPLIST`; do
 echo '---'$i'---'
 apktool-trds b $i 2>&1 | grep -v 'has no default translation' 1>&2
 rm $i/dist/$i.apk
